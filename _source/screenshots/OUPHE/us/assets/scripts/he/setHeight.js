/* config */
var heightUnit = "px"; // px or em are supported
var heightClasses = new Array();
var tallestElements = new Array();
heightClasses["frameworkHeight"] = new Array("c1","c2","c3");
heightClasses["inlineColumnHeight"] = new Array("ic1","ic2");
heightClasses["bundleBoxHeight"] = new Array("bundleBox1","bundleBox2","bundleBox3","bundleBox4","bundleBox5");

var d = document; 

function getTallestHeight( heightClassName ){
	// set the heights
	var box = heightClasses[heightClassName]; //list of DIV ids
	var h = 0;
	tallestElements[heightClassName] = "";
	for(x=0;x<box.length;x++){
		if( d.getElementById(box[x]) ){
			...
			test_h = parseInt(div.offsetHeight);
			d_alert("height: '" + test_h + "' > '" + h + "'");
			if( test_h > h ){
				h = test_h;
				d_alert("setting tallestElement: " + box[x]);
				tallestElements[heightClassName] = box[x];
			}
		}
	}
	return h;
...
function pxToEms( pixels ){
	return Math.round((pixels / 16)*10)/10;
}

function matchHeights( heightClassName, height ){
	// THIS IS THE OLD STUFF...USED FOR IE...
	var units = (height!="auto")?heightUnit:"";
	var h = height;
	// set the height of all divs to the tallest
	var box = heightClasses[heightClassName];
	if( heightUnit == "em" ){
		h = pxToEms( h );
	}	
	for(x=0;x<box.length;x++){
		if( d.getElementById(box[x]) ){
			if( x != 0 || box[x] != tallestElements[heightClassName] ){
				div = d.getElementById(box[x]);
				div.style.height = h + units; 
				div.style.scrollHeight = h + units;
			}
		}
	}
}
	...
	if( node != null ){
		...
		if( childItems != null ){
			for( i=0; i<childItems.length; i++ ){ 
				node.removeChild( childItems[i] );
			}
		}
	}
}

function writeHeightStyle( heightClassName, height ){
	var units = (height!="auto")?heightUnit:"";
	var headObjs = document.getElementsByTagName("head");
	if( headObjs != null ){
		var headObj = headObjs[0];
		var styleObj = document.getElementById( "heightStyles" );
		if( styleObj == null ){
			styleObj = document.createElement( "style" );
			styleObj.setAttribute( "type", "text/css" );
			styleObj.setAttribute( "id", "heightStyles" );
		}
		var box = heightClasses[heightClassName]; //list of DIV ids
		var selectors = "";
		var comma = "";
		for( i = 0 ; i < box.length; ++i ){
			if( i != 0 || box[i] != tallestElements[heightClassName] ){
				selectors = selectors + comma + "#" + box[i];
				comma = ", ";
			}
		}
		d_alert("setting height for: " + selectors);
		var styleContent = document.createTextNode( selectors  + " {\n" 
																						+ "  height: " + height + units + ";\n"
																						+ "  scrollHeight: " + height + units + ";\n"
																						+ "  }" );
		headObj.appendChild( styleObj );
		// IE barfs here. Does not allow style to have node appended?
		...
		styleObj.appendChild( styleContent ); 
	}
}
...
function setAllHeights() {
	removeChildren( "heightStyles" );
	for (var i in heightClasses){
		if( is_ie ){
			// have to reset these first or 
			// doesn't work on resize...
			matchHeights( i, "auto" );
			// now set what we need.
			var height = getTallestHeight(i);
			matchHeights( i, height );
		}
		else {
			// have to reset these first or 
			// doesn't work on resize...
			writeHeightStyle( i, "auto" );
			// now set what we need.
			var height = getTallestHeight(i);
			writeHeightStyle( i, height );
		}
	}
	if( d_alert_buffer ){
		//alert(d_alert_buffer);
		d_alert_buffer = "";
	}
}
...
//}
	...
	// document.getElementById( "c1" ).addEventListener("resize", test, false );
	//	}
